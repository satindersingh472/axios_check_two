...
function bring_chicken(details) {
    axios.request({
...
let bring_chick = document.getElementById(`bring_chick`)
bring_chick.addEventListener(`click`,bring_chicken);

function success_indian(response){
    for(let i = 0; i<response[`data`][`meals`].length;i++){
        box[`innerHTML`] += `<h2>Name: ${response[`data`][`meals`][i][`strMeal`]}</h2>`;
    }
}
function failure_indian(error){
box[`innerHTML`] += `<h2>error</h2>`;
}
function indian_food(details){
    axios.request({
        url: `https://www.themealdb.com/api/json/v1/1/filter.php`,
        params: {
            a: `Indian`
        }
    }).then(success_indian).catch(failure_indian);
}
let indian = document.getElementById(`indian`);
indian.addEventListener(`click`,indian_food);